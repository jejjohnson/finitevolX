...
- Linting via **ruff** (`uv run ruff check finitevolx/`)
- Type-hint checking via **ty** (`uv run ty check finitevolx`)
...
| Logging | `loguru` |
| CLI | `cyclopts` |
| Data containers | `dataclasses` (stdlib), `attrs`, or `equinox` |
| Configuration | `hydra-core` / `omegaconf` |
| Path handling | `pathlib` (stdlib) |
| HTTP | `httpx` |
| Testing | `pytest` |
| JAX computations | `jax`, `jaxtyping`, `equinox` |
