...
class TestWindStress3D:
    ...
    def test_top_level_injection(self, grid):
        ...
        _assert_only_level_nonzero(du, 1)
        _assert_only_level_nonzero(dv, 1)
        du_2d, dv_2d = WindStress2D(grid.horizontal_grid(), rho0=1000.0)(
            tau_x, tau_y, dz_top=10.0
        )
        np.testing.assert_allclose(du[1], du_2d)
        np.testing.assert_allclose(dv[1], dv_2d)
        np.testing.assert_allclose(du[1, 1:-1, 1:-1], 0.1 / (1000.0 * 10.0))
    ...
    def test_spatially_varying_thickness(self, grid):
        ...
        np.testing.assert_allclose(du[1, 1:-1, 3], 1.0 / 20.0)
...
class TestTransforms3D:
    def test_jit(self, grid):
        ...
        np.testing.assert_allclose(du[grid.Nz - 2, 1:-1, 1:-1], -0.5)
